...
import { brandsMenu, categoryMenu, citiesMenu } from '../../data/filterBarData';
...
const FiltersProvider = ({ children }) => {
    ...
    const applyFilters = () => {
        ...
        // filter by Cities
        const checkedCitiesItems = state.updatedCitiesMenu.filter(item => {
            return item.checked;
        }).map(item => item.label.toLowerCase());

        if (checkedCitiesItems.length) {
            updatedProducts = updatedProducts.filter(item => checkedCitiesItems.includes(item.city.toLowerCase()));
        }
    ...
    }, [state.sortedValue, state.updatedBrandsMenu, state.updatedCitiesMenu, state.updatedCategoryMenu, state.selectedPrice]);
    ...
    const handleCitiesMenu = (id) => {
        return dispatch({
            type: 'CHECK_CITIES_MENU',
            payload: { id }
        });
    };
